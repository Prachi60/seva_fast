...
export const verifyOnlinePaymentSchema = Joi.object({
  merchantOrderId: Joi.string().trim().optional(),
...
}).or("merchantOrderId", "razorpay_order_id");
...
export const updateDeliverySettingsSchema = Joi.object({
  ...
  adminCommissionPercent: Joi.number().min(0).max(100).optional(),
  technicalChargePercent: Joi.number().min(0).max(100).optional(),
  subAdminCommissionPercent: Joi.number().min(0).max(100).optional(),
  fieldWorkerCommissionPercent: Joi.number().min(0).max(100).optional(),
  goldCardMemberDiscountPercent: Joi.number().min(0).max(100).optional(),
  silverCardMemberDiscountPercent: Joi.number().min(0).max(100).optional(),
  bronzeCardMemberDiscountPercent: Joi.number().min(0).max(100).optional(),
  directSlabCommissionPercent: Joi.number().min(0).max(100).optional(),
  deductShippingBeforeCommission: Joi.boolean().optional(),
  advertiseChargePercent: Joi.number().min(0).max(100).optional(),
